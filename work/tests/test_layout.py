...
from layout.deck_parser import (
    KIND_BENCHMARKS, KIND_TOC, parse_deck,
)
...
def test_image_box_matches_the_layout():
    ...
    from sources import clean_images as S
...
def test_epigraph_box_sits_top_right():
    ...
    rect = R.epigraph_rect("short one")
    assert rect.x > L.SLIDE_W / 2
    assert abs(rect.x + rect.w - (L.SLIDE_W - L.MARGIN)) < 0.001
    assert rect.y + rect.h <= L.BAND_TOP + 0.5
...
def test_code_is_fixed_width_nine_point_blue():
    ...
    from gslides import render as R
    assert R.CODE_SIZE == 9
    assert R.CODE_BLUE == (0x3C, 0x78, 0xD8)
    assert R.CODE_FONT == "Consolas"
...
def test_month_names_agree():
    ...
    from layout import bench_page
    from gslides import deck as gdeck
    assert bench_page.MONTHS == gdeck.MONTHS
...
def test_bullet_inset_agrees():
    ...
    default = inspect.signature(S.hanging_indent).parameters[
        "indent"].default
    assert abs(L.BULLET_INSET - default) < 1e-9
...
def make_section(blocks, title="News"):
    ...
    from layout.deck_parser import Section
