...
with "test_". Layout and measurement live in
tests/test_layout.py.

Usage:
    python3 -m tests.test_deck
...
def test_deleted_counts_are_reported():
    ...
    from layout.deck_parser import deleted_count
...
def test_sidecar_path_is_derived():
    ...
    from layout.deck_parser import deleted_sidecar_path
...
def test_sidecar_topics_are_read():
    ...
    from layout.deck_parser import parse_deleted_topics
...
def test_missing_sidecar_is_not_an_error():
    ...
    from layout.deck_parser import read_deleted_topics
...
def test_recreated_match_is_forgiving():
    ...
    from layout.deck_parser import recreated_topics
    deck = parse_deck(
        "# T\n## slide: News\n"
        "###   the MUSIC industry   stops fighting\n- x\n"
    )
...
def test_topic_matching_cases():
    ...
    from layout.deck_parser import same_topic
...
def test_markup_splits_into_styled_pieces():
    ...
    from layout.deck_parser import split_markup
    pieces = split_markup(
...
def test_strip_markup_leaves_only_words():
    ...
    from layout.deck_parser import strip_markup
    assert strip_markup("a **b** `c` ==d==") == "a b c d"
    assert strip_markup("plain text") == "plain text"


# --------------------------------------------------------------
def test_markup_styles_all_render():
    """Every inline style the parser knows has a look."""
    from layout import deck_parser as P
    from gslides import render as R
    plain = R.piece_style(P.STYLE_PLAIN, 12)
    for style in (P.STYLE_BOLD, P.STYLE_HILITE, P.STYLE_CODE,
                  P.STYLE_RED):
        assert R.piece_style(style, 12) != plain, style


# --------------------------------------------------------------
def test_red_markup_is_its_own_style():
    """!!text!! marks a line for bold red."""
    from layout.deck_parser import split_markup, strip_markup
