#!/usr/bin/env python3
"""
Deck data objects, inline markup and topic matching.

The Google Slides workflow uses this module for its data
classes (Block, Section), for inline markup in bullets
(split_markup, strip_markup, is_link) and for deciding
whether two headlines name the same topic (same_topic).

It also still parses the Markdown deck format of the
earlier, cancelled workflow; sources/fetch_images.py and
sources/clean_images.py read it on their command line.

Turns a deck file into plain data objects that carry no
...
deck. `read_deleted_topics` returns those headlines. The
older in-file `<!-- deleted -->` flag is
...
Usage:
    from layout.deck_parser import parse_deck, parse_deck_file
...
# Inline markup inside a bullet. gslides.render turns each
# piece into a styled span; test_markup_styles_all_render
# checks every style has a look.
