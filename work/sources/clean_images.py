...
The Google Slides workflow uses run_convert and Settings
through sources/pictures.py, one picture at a time. The
command line below dates from the cancelled Markdown
workflow and reads a deck .md file.
...
renderer adds its own. The renderer fits pictures into
...
Usage:
    python3 -m sources.clean_images
    python3 -m sources.clean_images --ppi 96
    python3 -m sources.clean_images --force
    python3 -m sources.clean_images images_raw/gpt6-astra.png
...
from layout.deck_parser import DeckError, parse_deck_file
...
def collect_targets(argv):
    ...
    if not os.path.isdir(RAW_DIR):
        log_message(
            ...
            f"Run python3 -m sources.fetch_images first."
        )
        sys.exit(1)
